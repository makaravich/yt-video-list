...
class ImcaYoutubeFeedCore
{
    ...
    public function get_video_page_data($page_no = 0, $items_per_page = 0): stdClass
    {
        ...
        if ((int)$page_no > 0) $res->prevPageNum = $page_no; else $res->prevPageNum = '';

        $nextElemIdx = (($page_no + 1) * $items_per_page);
        if (isset($all_video_items[$nextElemIdx])) $res->nextPageNum = $page_no + 2; else $res->nextPageNum = '';
    ...
    private function render_video_list($videos)
    {
        ...
        <div class="imca-videos-list">
            ...
            <div class="nav-buttons">
                <?php echo $this->render_nav_buttons($prev_page_num, $next_page_num); ?>
            </div>
